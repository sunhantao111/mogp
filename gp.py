...
#向集合中添加一个短暂常量
    def addEphemeralConstant1(self, name, ephemeral, ret_type):
        """Add an ephemeral constant to the set. An ephemeral constant
        is a no argument function that returns a random value. The value
        of the constant is constant for a Tree, but may differ from one
        Tree to another.
...
######################################
def compile(expr, pset):# 如果原始集合有一个或多个参数，则返回一个函数，否则返回通过计算树得到的结果
    ...
    """
    code = str(expr)
    if len(pset.arguments) > 0:#判断原始集合中是否有参数
        # This section is a stripped version of the lambdify
        # function of SymPy 0.6.6.
        # args = ",".join(arg for arg in pset.arguments)
        for f in reversed(pset.arguments):#查找变量 f 是否存在。如果存在，就将其替换为一个形如 "list[index]" 的字符串，其中 index 是从 f 中去掉第一个字符后的字符串。
            if code.find(f) != -1:#code.find(f) 返回 f 在 code 中第一次出现的位置的索引，如果 f 不在 code 中则返回 -1 
                index = f[1:]#从变量 f 中去掉第一个字符后，将剩下的部分赋给变量 index 是一个字符串
                replace_str = "list" + "[" + index + "]"#构建一个列表索引
                # print(str)
                code = code.replace(f, replace_str)
        code = "lambda list: {code}".format(code=code)#使用{code}作为表达式,字符串的格式化操作 原始的 code 被嵌入到一个 lambda 函数的字符串中，形成了一个可以通过 eval 函数执行的可执行代码。这通常用于动态地构建并执行函数。
        #这个操作的目的是将表达式中的特定变量（如 "X1"）替换为一个与列表索引相关的形式。这种替换可能是为了使表达式能够接受一个列表作为输入，其中列表的索引对应于变量在列表中的位置
    try:
        return eval(code, pset.context, {})#pset.context 可能是一个包含变量或函数的字典
    except MemoryError:
...
def genFull1(pset1, min_, max_, type_=None):
    """Generate an expression where each leaf has the same depth
    between *min* and *max*.
    ...
    return generate1(pset1, min_, max_, condition, type_)
...
def generate1(pset1, min_, max_, condition, type_=None):
    """Generate a tree as a list of primitives and terminals in a depth-first
    order. The tree is built from the root to the leaves, and it stops growing
    the current branch when the *condition* is fulfilled: in which case, it
    back-tracks, then tries to grow another branch until the *condition* is
    ...
    while len(stack) != 0:
        ...
        if condition(height, depth):
            try:
                term = random.choice(pset1.terminals[type_])
            except IndexError:
                _, _, traceback = sys.exc_info()
                raise IndexError("The gp.generate function tried to add "
                                  "a terminal of type '%s', but there is "
            ...
            try:
                prim = random.choice(pset1.primitives[type_])
            except IndexError:
                _, _, traceback = sys.exc_info()
                raise IndexError("The gp.generate function tried to add "
                                  "a primitive of type '%s', but there is "
...
def cxOnePoint(ind1, ind2):#返回两个树的元组形式
    ...
    """
    if len(ind1) < 2 or len(ind2) < 2:#如果树的节点数量小于2,无法进行交叉,直接返回原树
        # No crossover on single node tree
        return ind1, ind2

    # List all available primitive types in each individual.列举每个个体中所有可用的基本类型
    types1 = defaultdict(list)  #types1,2是两个默认的字典
    types2 = defaultdict(list)
    if ind1.root.ret == __type__:#__type__是一个占位符,代表基本类型
        # Not STGP optimization
        types1[__type__] = list(range(1, len(ind1)))#生成了从1到树节点数（不包括根节点）的索引列表。
        types2[__type__] = list(range(1, len(ind2)))
        common_types = [__type__]#common_types 是一个包含 __type__ 的列表，表示两个个体共有的基本类型。在这种情况下，它的长度是1，因为唯一的共有类型是 __type__
    else:
        ...
        #print(type(common_types))

    if len(common_types) > 0:#基于共同类型的随机点交叉策略
...
def cxOnePointLeafBiased(ind1, ind2, termpb):#单点交叉策略,叶子节点可能被选择进行交叉
    """Randomly select crossover point in each individual and exchange each
    subtree with the point as root between each individual.
...
# GP Mutations                       #
######################################
def mutreplace_subtree(individual,ST_fit1_tuple):
    index=random.randrange(len(individual))
    slice_ = individual.searchSubtree(index)
    individual[slice_] = ST_fit1_tuple[1]
    
def mutUniform(individual, expr, pset):#均匀变异
    ...
    """
    index = random.randrange(len(individual))#随机选择个体中的一个索引
    # print(index)
    slice_ = individual.searchSubtree(index)
    # print(slice_)
    # print(individual)
    # print('--------------------------')
    type_ = individual[index].ret#获取该节点的返回类型
    individual[slice_] = expr(pset=pset, type_=type_)
    return individual,

def mutUniform1(offspring,expr, pset,i,ST_fit1_tuple,ST_fit2_tuple,ST_fit3_tuple,max_IGR_index):#均匀变异
    """Randomly select a point in the tree *individual*, then replace the
    subtree at that point as a root by the expression generated using method
    :func:`expr`.
    ...
    if ST_fit3_tuple[i] >= 0.5:
        #正常变异
        index = random.randrange(len(offspring[i]))
        slice_ = offspring[i].searchSubtree(index)
        type_ = offspring[i][index].ret#获取该节点的返回类型
        offspring[i][slice_] = expr(pset=pset, type_=type_)
    else:
        #if #左子树列表> 右子树列表：
        if ST_fit1_tuple[i] >=ST_fit2_tuple[i]:
            index= 2
        else:
            index = 1
        slice_1= offspring[i].searchSubtree(index)
        slice_2= offspring[max_IGR_index].searchSubtree(0)
        offspring[i][slice_1] = offspring[max_IGR_index][slice_2]



    return offspring[i],

#用于执行随机删除的操作，该函数的作用是随机删除个体树中的一个非终端节点，并用给定的表达式（expr）生成的子树替换该节点作为根节点
def drop_out(individual,expr,pset):
    ''' Randomly cut a node(not terminal) in the tree *individual*, then replace the
    subtree at that node as a root by the expression generated using method
    :func:`expr`.
    ...
    individual[slice_] = expr(pset=pset,type_=type_)
    return individual,
def mutNodeReplacement(individual, pset):#进行节点替换的变异操作
    ...
    index = random.randrange(1, len(individual))#通过随机选择确定个体的一个节点,这个节点可以是一终端节点也可以是一个基本操作符
    node = individual[index]
...
#随机替换——是用上一代最优个体的输出值
def mutNodeReplacement1(expr,individual,pset1):
    # if len(individual) < 4:
    #     return individual
    # index = random.randrange(1,len(individual))
    # node = individual[index]
    ...
    #     type_ = individual[index].ret
    # slice_ = individual.searchSubtree(index)
    # individual[slice_] = Terminal(random_value,False, type_)
    # return individual,
    if len(individual) < 4:
        return individual
    index = random.randrange(1,len(individual))
    ...
    type_ = individual[index].ret
    slice_ = individual.searchSubtree(index)
    individual[slice_] = expr(pset1=pset1,type_=type_)
    return individual
...
def mutShrink(individual):#individual表示待缩小的树的结构 这样的操作可能用于优化或演化算法中，通过缩小个体（树结构）的规模，以期望在搜索空间中找到更优解
    ...
    :returns: A tuple of one tree.
    """
    #该操作通过随机选择树结构中的一个分支，并将其替换为该分支的其中一个参数来缩小树的规模。具体来说，这是针对一个树形结构的操作，该树可能由多个分支（或节点）组成，每个分支可能有多个参数。通过执行这个操作，系统会随机选择一个分支，然后再从该分支的参数中随机选择一个，用选定的参数替换原来的分支，从而实现对整个树结构的缩小。
    # We don't want to "shrink" the root
    if len(individual) < 3 or individual.height <= 1:
        return individual,

    iprims = []
    for i, node in enumerate(individual[1:], 1):#找到满足条件的节点
